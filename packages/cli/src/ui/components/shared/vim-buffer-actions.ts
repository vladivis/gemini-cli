...
export function handleVimAction(
  ...
  const { store, cursorRow, cursorCol } = state;
  ...
  switch (action.type) {
    ...
    case 'vim_change_word_forward': {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (nextWord) {
          ...
          const currentLine = store[endRow] || '';
    ...
    case 'vim_change_big_word_backward': {
      ...
      for (let i = 0; i < count; i++) {
        const prevWord = findPrevBigWordAcrossLines(
          store.split('\n'),
          startRow,
          startCol,
        );
    ...
    case 'vim_change_word_end': {
      ...
      for (let i = 0; i < count; i++) {
        const wordEnd = findNextWordAcrossLines(
          store.split('\n'),
          row,
          col,
          false,
      ...
      if (endRow < store.length) {
        const lineLen = cpLen(store[endRow] || '');
    ...
    case 'vim_change_big_word_end': {
      ...
      for (let i = 0; i < count; i++) {
        const wordEnd = findNextBigWordAcrossLines(
          store.split('\n'),
          row,
          col,
          false,
    ...
    case 'vim_delete_line': {
      const { count } = action.payload;
      if (store.length === 0) return state;

      const linesToDelete = Math.min(count, store.length - cursorRow);
      const totalLines = store.length;
    ...
    case 'vim_change_line': {
      ...
      const linesToChange = Math.min(count, store.length - cursorRow);
      const nextState = detachExpandedPaste(pushUndo(state));

      const { startOffset, endOffset } = getLineRangeOffsets(
        cursorRow,
        linesToChange,
        nextState.store.split('\n'),
      );
      ...
      switch (movement) {
        ...
        case 'l': {
          ...
          return replaceRangeInternal(
            ...
            cursorCol,
            cursorRow,
            Math.min(cpLen(store[cursorRow] || ''), cursorCol + count),
            '',
          );
        }
    ...
    case 'vim_move_left': {
      const { count } = action.payload;
      const { cursorRow, cursorCol, store } = state;
    ...
    case 'vim_move_right': {
      ...
      for (let i = 0; i < count; i++) {
        const currentLine = store[newRow] || '';
    ...
    case 'vim_move_down': {
      const { count } = action.payload;
      const { cursorRow, cursorCol, store } = state;
      const newRow = Math.min(store.length - 1, cursorRow + count);
      const targetLine = store[newRow] || '';
      const targetLineLength = cpLen(targetLine);
      const newCol = Math.min(
        cursorCol,
    ...
    case 'vim_move_word_backward': {
      ...
      for (let i = 0; i < count; i++) {
        const prevWord = findPrevWordAcrossLines(store.split('\n'), row, col);
        if (prevWord) {
          row = prevWord.row;
          col = prevWord.col;
        } else {
    ...
    case 'vim_open_line_below': {
      const { cursorRow, store } = state;
      const nextState = detachExpandedPaste(pushUndo(state));

      // Insert newline at end of current line
      const endOfLine = cpLen(store[cursorRow] || '');
      return replaceRangeInternal(
        nextState,
        cursorRow,
        endOfLine,
    ...
    case 'vim_append_at_line_end': {
      const { cursorRow, store } = state;
      const lineLength = cpLen(store[cursorRow] || '');
    ...
    case 'vim_move_to_line': {
      ...
      const { store } = state;
      const targetRow = Math.min(Math.max(0, lineNumber - 1), store.length - 1);
